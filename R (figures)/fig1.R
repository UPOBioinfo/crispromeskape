...
fd <- ggplot(df4, aes(x = Bacteria, y = Frequency, fill = Group)) +
  ...
  scale_fill_manual("CRISPR-Cas type", values = getPalette) + theme_minimal() +
  ...
  #+ geom_text(mapping = aes(label = Frequency), size = 5, colour = 'black', vjust = 0, hjust = 0.5, position = position_fill())
print(fd)

df5 <- as.data.frame(readxl::read_excel("SupplFiles/SupplTableS1_95_95.xlsx", sheet = "spacers"))
...
# Figure
out1 <- ggarrange(fa, fb, fc, fd,
                 labels = c("a)", "b)", "c)", "d)"), 
                 font.label = list(size = 16, color = "black", face = "bold"), nrow = 2, ncol = 2)
out <- ggarrange(out1, fe, labels = c("", "e)"), font.label = list(size = 16, color = "black", face = "bold"), 
                 heights = c(3,1), nrow = 2, ncol = 1)          

tiff("Figures/fig1.tiff", width = 19, height = 18, units = "in", compression = "lzw", res = 300)
print(out)
dev.off()
