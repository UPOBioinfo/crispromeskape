...
# %%
species = ["_ec"] #["_sa",  "_pa", "_ef", "_ec", "_kp", "_ab"]
...
for s in species:
    ...
    _linkages = ['ward']
    labellist = ["{:>50}".format(l) for l in data_MLST.index.to_list()]
    ...
    # # get matrix of names and flatten to index flat distance matrices
    # name_matrix = data_MLST.apply(lambda x: x.name+'VS'+x.index)
    # # make triangle of lower matrix
    # name_matrix = name_matrix.where(
    #     np.tril(np.ones(name_matrix.shape)).astype(np.bool))
    # # fill nas
    # name_matrix = name_matrix.fillna("")
    # # add matrix with its transposed variant
    # name_matrix = name_matrix + name_matrix.T
    # # remove duplicates with regex
    # name_matrix = name_matrix.apply(
    #     lambda x: x.str.replace(r'\b(\w+)(\1)+\b', r'\1'), axis=1)
    # # finally: flatten, ignore that diagonal is not 0 (checks=false)
    # name_matrix_flat = squareform(name_matrix, checks=False)
    # # plot preprocessing
    # df_ml = pd.DataFrame({"Comparison": name_matrix_flat,
    #                       "Jaccard distance": data_MLST_nonsquare,
    #                       "Phylogenic distance": ab_phylogenic_distance_flat,
    #                       })
    # df_ml["mlsts"] = df_ml.Comparison.str.split("VS")
    # df_ml["Crispr-Status"] = ["Both Crispr" if (mlst_map[x[0]] != "No_crispr" and mlst_map[x[1]] != "No_crispr") else "One Crispr" if (
    #     mlst_map[x[0]] != "No_crispr" or mlst_map[x[1]] != "No_crispr") else "None Crispr" for x in df_ml.mlsts.to_list()]
    # # TODO: use mlsts list of groups to index mlst_map and get info if one or both of the mlsts contains a crispr group
    # # plot
    # sns_palette = sns.color_palette(colors_hex[1:4])
   
    # fig, ax = plt.subplots(figsize=(10, 10))
    # sns.scatterplot(data=df_ml,
    #                 x="Phylogenic distance",
    #                 y="Jaccard distance",
    #                 hue="Crispr-Status",
    #                 markers=['o'],
    #                 palette=sns_palette,
    ...
    # ax.set_xlabel("Phylogenic distance")
    # fig.savefig("output/{}_jacc-vs-phylo-lmplot.png".format(s))
    ...
    for _linkage in _linkages:
        ...
        with plt.rc_context({'lines.linewidth': 2.5}):
            ...
            #plt.show()
            fig.savefig(
                "output/dendro_{}_{}-linkage-phylogenic.svg".format(s, _linkage), dpi=300)
# save correls
